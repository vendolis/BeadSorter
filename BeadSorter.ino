...
// calibration 
float thresholdH = 0.026;// 0.030;  // hue tolerance
float thresholdS = 0.023;// 0.025;  // saturation tolerance
float thresholdL = 0.010;// 0.014;  // lightness tolerance
uint16_t thresholdC = 300; // clear channel tolerance

// Null-scan window: maximum deviation from empty-tube reference in each channel.
float nullScanOffsetH = 0.008;
float nullScanOffsetS = 0.01;
float nullScanOffsetL = 0.008;
...
void loop() {
  ...
  if (digitalRead(setupPin) == HIGH) {
    ...
    while (digitalRead(setupPin) == LOW) {}  // wait for next press
    while (digitalRead(setupPin) == HIGH) {} // wait for release
    delay(50);
